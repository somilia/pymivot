...
class DatabasePSQL:
    ...
    def _drop_table(self, model_name, schema_name="public", is_model=True):
        try:
            query = f'DROP TABLE IF EXISTS "{schema_name}".{self._get_table_name(model_name, is_model)} CASCADE;'
            self.execute_query(query)
            print(f'Table "{schema_name}".{self._get_table_name(model_name, is_model)} dropped successfully.')
        except psycopg2.Error as e:
            print("Error during the connection to the database:", e)
    ...
    def insert_and_verify_data(self, table_name, schema_name, column_names, values, is_model=True):
        ...
        if not self._table_exists(table_name, schema_name, is_model=is_model):
            raise TableDoesNotExistException(f"Table {schema_name}.{self._get_table_name(table_name, is_model)} does not exist.")

        # Get table tap_schema
        ...
        # Check if provided column names match with table tap_schema
        ...
        if not set(column_names).issubset(set(schema_column_names)):
            raise NotMatchingSchemaException("Provided column names do not match with table tap_schema.")
        ...
        if values not in fetched_data.data:
            ...
            print(" --- Values :", values)
            raise NotMatchingSchemaException("Data verification failed after insertion.")
    ...
    def create_schema(self, schema_name):
        if self.check_schema_exist(schema_name):
            raise SchemaAlreadyExistsException(f'Schema "{schema_name}" already exists.')
        try:
            ...
            print("Error during the creation of the tap_schema: ", e)
    ...
    def drop_schema(self, schema_name):
        try:
            ...
            print("Error during the drop of the tap_schema: ", e)
    ...
    def check_schema_exist(self, schema_name):
        try:
            ...
            print("Error during the check of the tap_schema existence:", e)
            return False
